                ...
                <div class="row">
                    <div class="col-lg-5">
                        <div class="auth-form-sidebar">
                            <img height="142px" width="142px" src="{{ asset($app->logo) }}" alt="{{ $app->title }}">
                            <h5 class="fw-bold mt-4">{{ __('LinkProfilde') }}</h5>
                        </div>
                    </div>
                        ...
                        <form class="auth-form" method="POST" action="{{ route('password.email') }}">
                            ...
                            @if(session()->has('status'))
                                <div class="alert alert-primary" role="alert">
                                    {{ __('Hesabınız varsa şifre sıfırlama bağlantısı e-posta adresinize gönderildi.') }}
                                </div>
                            @endif

                            <h5 class="mb-4">{{ __('Şifreyi Sıfırla') }}</h5>
